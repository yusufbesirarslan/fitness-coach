"""AxisAI UIUX Sprint 1 PR1 — kanonik gezinme sözleşmesi (app/nav.py) testleri.

Saf birim testleri: Flask uygulaması/DB gerektirmez. Sözleşmenin TEK doğruluk
kaynağı olduğunu ve hiçbir iş kuralı taşımadığını doğrular.
"""
import pytest
...
def test_nutrition_is_secondary_not_primary():
    ...
    assert "nutrition" in secondary


def test_community_destinations_are_secondary():
    primary = {d["id"] for d in nav.primary_destinations()}
    secondary = {d["id"] for d in nav.secondary_destinations()}
    for cid in ("feed", "friends", "leaderboard", "quests", "challenges", "gallery"):
...
def test_resolve_active_maps_pages_and_child_routes_to_primary():
    r = nav.resolve_active
    assert r("today") == "today"
    assert r("home") == "today"          # legacy 'home' işareti → Today
    assert r("plan") == "plan"
    assert r("training") == "plan"       # Plan alias/çocuk rota → Plan aktif
    assert r("coach") == "coach"
    assert r("progress") == "progress"


def test_resolve_active_none_for_secondary_utility_and_unknown():
    r = nav.resolve_active
    assert r("nutrition") is None        # indirildi → hiçbir birincil aktif değil
...
def test_contract_rows_are_read_only():
    # MappingProxyType: sözleşme çalışma zamanında kazara değiştirilemez.
...
def test_every_row_has_required_presentational_keys():
    ...
    for d in list(nav.primary_destinations()) + list(nav.secondary_destinations()):
        ...
        # yol daima kök-göreli kanonik rota (uydurma/mutlak URL yok)
