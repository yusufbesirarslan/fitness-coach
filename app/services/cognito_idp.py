...
class CognitoIdpError(Exception):
...
def _wrap(exc):
    """boto3 ClientError'ı (varsa) Türkçe CognitoIdpError'a çevir."""
    code = ""
    try:
    ...
    if code:
        ...
        return CognitoIdpError(_ERROR_MESSAGES.get(code, "İşlem başarısız. Lütfen tekrar dene."), code)
    _logger.warning("[COGNITO-IDP] beklenmeyen hata: %s: %s", type(exc).__name__, exc)
    return CognitoIdpError("İşlem başarısız. Lütfen tekrar dene.")
...
def initiate_auth(username, password):
    ...
    döndürür. Doğrulanmamış e-posta → CognitoIdpError(code='UserNotConfirmed...').
    """
