...
def checkin():
    ...
    submission_key = read_idempotency_key() if raw_key is not None else None
    if raw_key is not None and submission_key is None:
        return jsonify({"error": t("route.checkin_invalid_key")}), 400
    ...
    if submission_key:
        fingerprint = hashlib.sha256(json.dumps({
            "weight": weight, "yogunluk": yogunluk, "fatigue": fatigue,
            "progressive_overload": overload, "uyku_kalitesi": uyku,
            "beslenme_uyumu": beslenme, "note": note,
    ...
    if submission_key:
        ...
        resp = {"message": t("route.checkin_saved"),
                "coach_feedback": coach_feedback}
        if quest_result:
