...
| Barcode log | `POST /api/food/barcode/add` | no — `@require_auth` | **DEPRECATED compatibility surface** (Sprint 13 PR3, C13) — still supported for now, responds `Deprecation: true` + `Link: </meal-log>; rel="successor-version"`. Removal requires C13 evidence. **Sprint 13 PR5 deferred removal** (no elapsed deprecation period). **no sunset date is promised**. F6 safety remains CLOSED by PR3. Writes `MealLog` with `source="barcode"` recomputed from server-resolved provider truth (a caller-supplied `food` object is no longer read), honours `Idempotency-Key`, and publishes no raw `MealLog.id` on any variant |
