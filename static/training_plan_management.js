 ...
 * and holds no version of its own. It owns four things and nothing else:
 *
 ...
 *   3. a canonical FRESHNESS PRECHECK before any destructive replacement; and
 *   4. the rule that a successful write is not reported as done until a
 *      canonical refresh has actually run.
 *
 * ── On the freshness precheck, honestly ──────────────────────────────────────
 * `POST /training-plan/save` is the app's one destructive TrainingPlan path: it
 * deletes every row for the owner and inserts a new lineage. It accepts NO
 * precondition — no expected version, no If-Match, no lineage guard — and
 * neither does the native generation command. That server contract is not
 * changed here, and this module does not pretend otherwise.
 *
 * What it does is re-read the canonical plan identity (`plan_lineage` +
 * `mutation_version`, the same pair the native contract publishes) immediately
 * before replacing, and REFUSE when it no longer matches the identity the page
 * was rendered against. Both readings are the SERVER's; the browser only
 * compares them and fails closed. That closes the window that actually exists
 * in practice — the seconds-to-minutes between "user decided to regenerate" and
 * "user confirmed", during which a Coach mutation or another tab can move the
 * plan. It does NOT close the sub-request window between the re-read and the
 * write; only a server-side precondition can, and that is recorded as a
 * prerequisite in docs/PLAN_DOMAIN_CONVERGENCE.md rather than faked here.
 */
(function (root, factory) {
  ...
  // Canonical endpoints. Owned HERE so a renderer cannot point the workflow at a
  // different generation or persistence path; no route is invented.
  var GENERATE_URL = '/training-plan';
  var SAVE_URL = '/training-plan/save';
  var CANONICAL_READ_URL = '/training/bootstrap';

  // Bounded management states (brief §39). `proposal_ready` and `active` are
  // deliberately different states with no transition between them that does not
  // pass through `saving` — a proposal can never become "active" by being looked
  // at, navigated away from, or refreshed.
  var IDLE = 'idle';
  ...
  // The one 400 the generate endpoint uses to mean "this user has no profile
  // session yet". Recognised here so both renderers classify it identically;
  // WHERE the user is sent to fix it stays a presentation decision.
  ...
  function baselineFrom(source) {
    ...
    // Idempotent: accepts either the server's field names (`plan_lineage` /
    // `mutation_version`, as the bootstrap and the Plan render emit them) or an
    ...
    var rawLineage = source.plan_lineage !== undefined
      ? source.plan_lineage : source.lineage;
  ...
  function createPlanManagement(options) {
     ...
     *
     * Order is the guarantee: prove freshness → write → canonical refresh. A
     * refusal or a failure at any step leaves the CURRENT plan untouched, and
    ...
    async function replace() {
      ...
      try {
        ...
        var verdict = compareBaselines(baseline, current);
        if (verdict === 'undecidable') return fail('freshness_unavailable', null);
        if (verdict === 'changed') {
          // Someone else moved the plan between render and confirmation. Refuse
          // and hand the decision back to the user against the CURRENT plan.
    ...
    return {
       ...
       * snapshot), never computed by a renderer. */
